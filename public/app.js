﻿﻿﻿﻿﻿﻿﻿const CLOUD_URL = 'https://script.google.com/macros/s/AKfycbzY63DzQTu_RP106fQoI2q0joumt_vJhhesMufpJN1iTZQtBoZRYWNs7wfb88xRp2PBsg/exec';
        ...
        function getAppsScriptAllowedOrigins() {
            const origins = new Set();
            try {
                const url = new URL(CLOUD_URL);
                if (url.origin) origins.add(url.origin);
            } catch (err) {}
        ...
        function postToAppsScriptForResponse(payload, timeoutMs = 12000) {
            return new Promise((resolve, reject) => {
                const messageId = `delta8_post_${Date.now()}_${Math.random().toString(36).slice(2)}`;
                const frameName = `delta8_post_frame_${Date.now()}_${Math.random().toString(36).slice(2)}`;
                ...
                const allowedOrigins = getAppsScriptAllowedOrigins();
                let settled = false;
                ...
                const handleMessage = (event) => {
                    ...
                    if (!payloadData || payloadData.ok === false) {
                        finishError((payloadData && payloadData.error) || 'Request gagal.');
                        return;
                    }
                    finishSuccess(payloadData);
                };

                const timer = setTimeout(() => {
                    finishError('Backend tidak merespons.');
                }, timeoutMs);
                ...
                try {
                    ...
                    finishError((err && err.message) || 'Gagal mengirim request.');
                }
            });
        }

        async function verifyAuthViaPost(payload) {
            const timeouts = [4000, 8000, 15000];
            let lastError = null;
            for (const timeoutMs of timeouts) {
                try {
                    return await postToAppsScriptForResponse({
                        action: 'verifyAuth',
                        pin: payload.pin,
        ...
        function applyAdminMode() {
            ...
            // Tambahkan class khusus ke body untuk CSS override yang lebih mudah
            document.body.classList.add('is-iframe-admin');
            ...
            // Reset style container agar tidak ada padding/margin ganda
        ...
        function normalizeTheme(t) {
            return ['emerald', 'platinum', 'crystal'].includes(t) ? t : 'emerald';
        }

        function setTheme(t) { 
            t = normalizeTheme(t);
            document.body.setAttribute('data-theme', t); 
            localStorage.setItem('delta8_theme', t); 
            document.querySelectorAll('.theme-dot').forEach(d => d.classList.remove('active')); 
            if (document.getElementById('dot-' + t)) document.getElementById('dot-' + t).classList.add('active'); 
            // Beritahu parent (admin panel) jika ada perubahan tema
            if (window.parent !== window) {
                window.parent.postMessage({ type: 'delta8-theme-change', theme: t }, '*');
            }
        }
        ...
        async function submitAuth() {
            ...
            try {
                ...
                const data = await verifyAuthViaPost({ pin, editor, deviceId });
...
async function syncAll() {
    ...
    bootstrap.Modal.getInstance('#modalCatat').hide();
        ...
        window.onload = () => {
    setTheme(localStorage.getItem('delta8_theme') || 'emerald');
    applyAdminMode();
    syncPaymentOptionAvailability();
    initTahun();
